...
> {
  ...
  public conceal = () => {
    ...
    if (
      secretData.width > carrierData.width ||
      secretData.height > carrierData.height
    ) {
      this.setState({
        error:
          'The secret image can not be larger than the carrier image either both width and height.',
      });
      return;
    }
  ...
  public extract = () => {
    ...
    for (let x = 0; x < resultData.width; x += 1) {
      for (let y = 0; y < resultData.height; y += 1) {
        ...
        for (let c = 0; c < 3; c++) {
          resultData.data[redIndex + c] =
            (extractData.data[redIndex + c] & (255 - 248)) << 5;
        }
  ...
  public imageUpload = (type: DropType, className?: string) => {
    ...
    return (
      <div className={'flex-no-grow w-1/2 h-32' + ' ' + className}>
        <Dropzone
  ...
  public render() {
    ...
    const valid = this.state.conceal
      ? this.state.carrier !== null && this.state.secret !== null
      : this.state.extract !== null;
    ...
    return (
      <div className="">
        <div className="flex flex-wrap-reverse pt-10 -ml-4">
          <div
            className={
              'flex-1 flex flex-col xs:text-lg sm:text-xl ml-4' + className
            }
          >
            ...
            <div className="flex-1 bg-black text-green-light p-4">
              <div className="flex justify-center">
                {this.state.conceal
                  ? this.imageUpload(DropType.Carrier, 'mr-2')
                  : this.imageUpload(DropType.Extract)}
              ...
              <div className="flex justify-between pt-2">
                ...
                <div className="flex-no-grow">
                  <Button
                    ...
                    unvalidClassName=""
                    childHover={
                      <div className="bg-green-light text-black px-2 py-1">
                        {this.state.conceal ? 'Conceal' : 'Extract'}
                      </div>
                    }
                    childNormal={
                      <div className="text-green-light px-2 py-1">
                        {this.state.conceal ? 'Conceal' : 'Extract'}
                      </div>
                    }
                    valid={valid}
                    onClick={() =>
                      this.state.conceal ? this.conceal() : this.extract()
                    }
                  />
                </div>
              </div>
          ...
          <div className="flex-1 ml-4">
            <div className="xs:text-xl sm:text-3xl text-grey-dark pb-4">
              Steganography
            </div>
            <div className=" pb-4">
              <p>
                Steganography is the art and science of hiding secret data
                within other data, known as the carrier. Here you can upload two
                images, and the secret image will be hidden within the carrier
                image. Or you can upload a single image to extract any secret
                images hidden within. Try it out!
              </p>
            </div>
          </div>
        </div>
