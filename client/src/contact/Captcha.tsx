...
class Captcha extends React.Component<CaptchaProps, CaptchaState> {
  ...
  public tryCaptcha = () => {
    ...
    api.post('generate_captcha', { key: this.state.input }).then(
      (resp) => {
        this.setState({
          valid: true,
          error: null,
  ...
  public captchaValue = () => {
    switch (this.state.captcha) {
      ...
      default:
        return (
          <div className="flex-no-grow flex-no-shrink text-captcha whitespace-pre font-bold">
            {this.state.captcha}
          </div>
        );
    }
  ...
  public render() {
    return (
      <div className={this.props.className + ' -ml-8'}>
            ...
            <div className="flex-no-grow flex">
              <div className="flex flex-col justify-end pr-2">
                <Button
                  ...
                  childNormal={
                    <img src={redowhite} alt="redo" className="h-4 w-4" />
                  }
                  childHover={
                    <img src={redoblack} alt="redo" className="h-4 w-4" />
                  }
                />
              </div>
